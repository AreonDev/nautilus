				 ...
				 NautilusQuery		*query)
{
	...
	g_free (current_text);
        ...
        if (g_set_object (&priv->query, query)) {
                ...
                /* When it the query is set and it's non-NULL, the query editor's
                 * location must be reassingned to the query's one.
                 */
                if (query) {
                        GFile *location;
                        ...
                        nautilus_query_editor_set_location (editor, location);

                        g_clear_object (&location);
                }

                nautilus_search_popover_set_query (NAUTILUS_SEARCH_POPOVER (priv->popover), query);
